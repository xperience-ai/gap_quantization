...
def conv_quant(module, cfg):
    ...
    params = {
        'norm':
            max(0, out_int_bits + w_frac_bits + inp_frac_bits - cfg['bits'] + cfg['signed']),
        'weight':
            integerize(module.weight.data, w_frac_bits, cfg['bits']).cpu().tolist(),
        'bias':
            integerize(module.bias.data, cfg['bits'] - out_int_bits - cfg['signed'],
                       cfg['bits']).cpu().tolist()
    }
